...
export const fetchPerkembangan = (url) => {
  console.log({url}, '<<< fetch perkembangan');
  return (dispatch) => {
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
        console.log(data, '<<<fetch perkembangan', url);
        dispatch({type: 'SET_PERKEMBANGAN', payload: data});
      })
      .catch((err) => {
...
export const fetchPerkembanganById = (id) => {
